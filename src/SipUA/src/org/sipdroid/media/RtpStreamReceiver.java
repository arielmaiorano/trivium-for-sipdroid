...
public class RtpStreamReceiver extends Thread {
	...
	public int speaker(int mode) {
		...
		if (mode == AudioManager.MODE_NORMAL
			...
			Toast.makeText(Receiver.mContext, R.string.help_speakerphone,
	...
	public static void adjust(int keyCode, boolean down) {
		AudioManager mAudioManager = (AudioManager) Receiver.mContext
				.getSystemService(Context.AUDIO_SERVICE);
			...
			if (SystemClock.elapsedRealtime() - down_time < 500) {
				...
				if (ogain > 1)
					if (keyCode == KeyEvent.KEYCODE_VOLUME_UP) {
						...
						if (mAudioManager.getStreamVolume(stream()) == mAudioManager
						...
						if (mAudioManager.getStreamVolume(stream()) == 0)
							return;
						gain = ogain;
					}
				mAudioManager
			...
			if (oldring > 0)
				setStreamVolume(
						...
						(int) (am.getStreamMaxVolume(AudioManager.STREAM_RING)
								* org.sipdroid.sipua.ui.Settings.getEarGain() * 3),
						0);
			track.setStereoVolume(
					AudioTrack.getMaxVolume()
							* (ogain = org.sipdroid.sipua.ui.Settings
									.getEarGain() * 2),
					AudioTrack.getMaxVolume()
	...
	void saveSettings() {
		if (!PreferenceManager.getDefaultSharedPreferences(Receiver.mContext)
				.getBoolean(org.sipdroid.sipua.ui.Settings.PREF_OLDVALID,
			...
			Editor edit = PreferenceManager.getDefaultSharedPreferences(
					Receiver.mContext).edit();
			edit.putInt(org.sipdroid.sipua.ui.Settings.PREF_OLDVIBRATE,
					oldvibrate);
			edit.putInt(org.sipdroid.sipua.ui.Settings.PREF_OLDVIBRATE2,
					oldvibrate2);
			edit.putInt(org.sipdroid.sipua.ui.Settings.PREF_OLDPOLICY,
					oldpolicy);
			edit.putInt(org.sipdroid.sipua.ui.Settings.PREF_OLDRING,
	...
	public static void restoreMode() {
		if (PreferenceManager.getDefaultSharedPreferences(Receiver.mContext)
				.getBoolean(org.sipdroid.sipua.ui.Settings.PREF_SETMODE,
	...
	void initMode() {
		...
		if (Receiver.call_state == UserAgent.UA_STATE_INCOMING_CALL
				&& (Receiver.pstn_state == null || Receiver.pstn_state
						.equals("IDLE")))
	...
	void newjitter(boolean inc) {
		if (good == 0 || lost / good > 0.01 || call_recorder != null)
			return;
		int newjitter = (int) Math.sqrt(devheadroom) * 5
				+ (inc ? minjitteradjust : 0);
		if (newjitter < minjitter)
			newjitter = minjitter;
		if (newjitter > maxjitter)
			newjitter = maxjitter;
		if (!inc
				&& (Math.abs(jitter - newjitter) < minjitteradjust || newjitter >= jitter))
			return;
